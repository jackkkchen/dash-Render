# https://www.etoro.com/discover/markets/stocks/exchange/nyse 仿照该网站的卡片案例制作单个app
...
#     symbol='MSFT', # MSFT是我们拉取的公司股票
#     interval='1min', # 设置每一分钟拉动一次
#     outputsize='compact' # 设置为compact，只拉取少数数据，全部的话为full
#     )  
# df = data.copy()
...
dff = dff[dff.indicator.isin(['high'])] #为方便展示，只取high，也就是股票的最高点
...
app.layout = dbc.Container([ #使用dbc容器构建layout
    ...
    dcc.Interval(id='update', n_intervals=0, interval=1000*5) #我们的间隔，每五秒更新一次app
])
